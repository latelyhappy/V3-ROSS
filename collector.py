...
def init_db():
    """
    初始化戰地數據庫。若檔案與資料表不存在則自動建立。[cite: 2]
    """
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # 建立動能事件表 (Momentum Events)[cite: 2]
    cursor.execute('''
...
def log_event(ticker, headline, float_m, price_initial, volume=0, change_percent=0.0):
    ...
    try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (now_sec, dt_str, ticker, headline, float_m, price_initial, price_initial, 0)) #[cite: 2]
...
def evaluate_sniper_tags(ticker, float_m, volume, rvol, change_percent, is_above_vwap=True):
    """
    [新增功能：UI 提醒標籤引擎]
    判斷該標的目前屬於哪個戰術狀態，供前端 index.html 渲染。
    """
    # 1. 💀 陷阱區過濾 (最高優先級)
    ...
    # 2. 🎯 狙擊點判定 (ROSS 級高勝率特徵)
    # 條件：量比>2、Float<5M、漲幅>2%、且在VWAP之上
...
def update_max_price(ticker, current_price):
    """
    盤中持續呼叫。檢查該股票在過去 15 分鐘內是否有記錄，[cite: 2]
    若有，且當前價格更高，則更新其 15 分鐘最高價 (price_max_15m)。[cite: 2]
    若超過 15 分鐘，則將該紀錄結案 (is_closed = 1)。[cite: 2]
    """
    try:
        conn = sqlite3.connect(DB_PATH, timeout=5.0)
        cursor = conn.cursor()
        ...
        # 1. 將超過 15 分鐘的紀錄結案[cite: 2]
        cursor.execute('''
            UPDATE momentum_events 
            SET is_closed = 1 
            WHERE is_closed = 0 AND timestamp_sec < ?
        ''', (fifteen_mins_ago,)) #[cite: 2]
        
        # 2. 找出該代碼目前仍在 15 分鐘監控期內的紀錄[cite: 2]
        cursor.execute('''
...
def generate_intelligence_summary():
    """
    [新增功能]：後端運算摘要引擎。[cite: 2]
    直接在資料庫中統計並回傳前 50 大高勝率、高漲幅詞彙的 JSON 報表。[cite: 2]
    減輕 AI 分析負擔。[cite: 2]
    """
    import re
    ...
    try:
        ...
        # 只抓取過去 3 天內的結案資料，避免歷史包袱[cite: 2]
        three_days_ago = time.time() - (3 * 24 * 60 * 60)
        cursor.execute('''
            SELECT headline, price_initial, price_max_15m 
            FROM momentum_events 
            WHERE is_closed = 1 AND timestamp_sec > ?
        ''', (three_days_ago,)) #[cite: 2]
        
        rows = cursor.fetchall()
        conn.close()

        stop_words = {'WITH', 'THAT', 'THIS', 'FROM', 'THEIR', 'WILL', 'HAVE', 'BEEN', 'WERE', 'AFTER', 'OVER', 'MORE', 'THAN', 'ABOUT', 'INC', 'CORP', 'LTD', 'THE', 'AND', 'FOR'} #[cite: 2]
        stats = {}
        ...
        for gram, data in stats.items():
            if data['occurrences'] >= 2: # 至少出現過2次才具備統計意義[cite: 2]
        ...
        # 依據平均漲幅排序，取前 50 名[cite: 2]
        summary = sorted(summary, key=lambda x: x['avg_gain_pct'], reverse=True)[:50] #[cite: 2]
